...
class TestResponseImportAliases:
    ...
    def test_response_import_works(self):
        ...
        from oguild.response import (
            CommonErrorHandler, DatabaseErrorHandler, ValidationErrorHandler,
            NetworkErrorHandler, AuthenticationErrorHandler, FileErrorHandler
        )
    ...
    def test_responses_import_works(self):
        ...
        from oguild.responses import Ok, Error, police
    ...
    def test_imports_are_identical(self):
        ...
        from oguild.response import (
            Ok as ResponseOk, Error as ResponseError, police as response_police,
            CommonErrorHandler as ResponseCommonHandler
        )
        from oguild.responses import (
            Ok as ResponsesOk, Error as ResponsesError, police as responses_police,
        ...
        assert ResponseError is ResponsesError, "Error classes should be identical"
        assert response_police is responses_police, "police functions should be identical"
        assert ResponseCommonHandler is ResponsesCommonHandler, "CommonErrorHandler classes should be identical"
    ...
    def test_module_direct_import(self):
        ...
        assert hasattr(responses_module, 'Ok'), "oguild.responses should have Ok"
        assert hasattr(response_module, 'Error'), "oguild.response should have Error"
        assert hasattr(responses_module, 'Error'), "oguild.responses should have Error"
        assert hasattr(response_module, 'police'), "oguild.response should have police"
        assert hasattr(responses_module, 'police'), "oguild.responses should have police"

    def test_response_functionality(self):
        """Test that response classes work through both import paths."""
        from oguild.response import Ok as ResponseOk, Error as ResponseError
        from oguild.responses import Ok as ResponsesOk, Error as ResponsesError
        ...
        try:
            response_error = ResponseError("Test error", _raise_immediately=False)
            responses_error = ResponsesError("Test error", _raise_immediately=False)
            ...
            pytest.fail("Error creation should not raise when _raise_immediately=False")
